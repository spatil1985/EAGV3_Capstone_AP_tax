...
**Also re-confirmed:** B7 (recurring bills regenerating daily) is **still live on
Keystone** — as of `2026-09-23` the three broken `RecurringBill` templates have now
produced **four** consecutive days of duplicate drafts (20/21/22/23 Sep), while the
control template with a future `next_bill_date` has still fired exactly once.
`next_bill_date`/`last_generated_date` are still not advancing.
**The same defect class is also live on Suryodaya — see N6.**

> ⚠️ **B7 itself has never been filed.** Verified 2026-09-23 via
> `GET /api/bug-report/mine` on both instances: 25 reports on India, **0 on
> Keystone**, and no B7/B8/B9 among them. N1–N8 below *are* all filed. See
> `agentswitch_submissions.md` §C — B7 is the top unfiled item.
...
SUMMARY
B7 (written up against Keystone/US in agentswitch_submissions.md §C -- NOTE: B7 was
never actually filed on either instance, verified 2026-09-23) described RecurringBill
