# AgentSwitch MCP Tool Inventory — US (Keystone) — live snapshot

Captured 2026-09-23 against `https://class.agentswitch.theschoolofai.in` via the real MCP handshake (`initialize` -> `notifications/initialized` -> `tools/list`).
...
- Company: `c1e47d8d-b849-4187-9a32-4103d3dece4a` (Keystone Precision Works LLC)
- Locale: US / sales_use_tax

**Total tools visible to this account/role: 468**, grouped into **142** entity/endpoint groups below.

Tools are scoped per-caller — this is not the platform's full tool catalog, only what this role + app entitlements + row scope permit (per the server's own `initialize` instruction string). See the companion [India/Suryodaya inventory](mcp_tool_inventory_india.md) for comparison.
